...
const Pokemon = ({pokemon}) => {
    ...
    const renderTypes = () => (
        pokemon.types.map(type => (
            <li key={type.slot} className="px-2 py-1 bg-rose-500 rounded">
                {type.type.name}
            </li>
        ))
    )
    ...
    const renderStats = () => (
        pokemon.stats.map((stat, index) => (
            <div key={index} className="bg-rose-100 my-2 rounded p-1">
                <div className="bg-cyan-900 rounded px-2" style={{width: `${stat.base_stat}%`}}>
    ...
    return (
        <Layout title={pokeName}>
            ...
            <div className="bg-rose-900 rounded p-5">
